...
let add1Arr = aea.liftA(add1);
let add2Arr = aea.liftA(add2);
let add5Arr = aea.liftA(add5);
...
//
let testFirstArrow = aea.firstA(aea.liftA((x) => x * 2))
testFirstArrow([2, "dave"], (x) => console.log('testFirstArrow: ', x), aea.p)// types for looping
//
let testSecondArrow = aea.secondA(aea.liftA((x) => x * 3))
...
//
let frst = aea.firstA(aea.liftA((x) => x + 1))
